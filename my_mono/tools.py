...
def _make_edit_tool(cwd: str) -> AgentTool:
    ...
    def execute(path: str, old_text: str, new_text: str, **_) -> str:
        ...
        if new_content == content_for_replacement:
            return (
                f"Error: no changes would be made to {path}. "
                f"The replacement produces identical content."
            )
...
def _make_grep_tool(cwd: str) -> AgentTool:
    ...
    def _grep_rg(
        ...
        glob: str | None, ignore_case: bool, literal: bool,
        context: int, limit: int,
    ) -> tuple[list[str], bool]:
        ...
        **_,
        ...
        if _rg_available():
            lines, limit_reached = _grep_rg(
                pattern, str(search_path), glob,
                ignore_case, literal, context, effective_limit,
            )
        else:
            lines, limit_reached = _grep_python(
                pattern, search_path, glob,
                ignore_case, literal, context, effective_limit,
            )
