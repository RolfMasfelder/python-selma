...
class Agent:
    ...
    def __init__(self, options: AgentOptions):
        ...
        logger.info("Agent initialized | model=%s tools=%s",
                    options.model, [t.name for t in options.tools])
    ...
    async def _run_loop(self) -> None:
        ...
        try:
            while True:
                ...
                logger.info("Turn %d | model=%s messages=%d",
                            turn, self._state.model, len(openai_messages))
            ...
            logger.info("Agent end | turns=%d messages=%d",
