...
def test_daily_memory_yesterday_loaded():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        (ws / "memory" / f"{yesterday}.md").write_text(
            "- Yesterday: important decision\n", encoding="utf-8"
        )
...
def test_memory_get_reads_full_file():
    ...
    from tools import _make_memory_get_tool
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        for evil_path in ["../../etc/passwd", "../selma.json", "/etc/hosts"]:
            ...
            assert "Error" in result, (
                f"Traversal attack '{evil_path}' was not blocked"
            )
...
def test_chunk_text_splits_at_paragraphs():
    ...
    from memory_index import _chunk_text
...
def test_build_fts_query_tokenizes_words():
    ...
    from memory_index import _build_fts_query
...
def test_search_tool_returns_formatted_output():
    ...
    from tools import _make_memory_search_tool

    with tempfile.TemporaryDirectory() as tmp:
        ws = _make_workspace(tmp)
        (ws / "MEMORY.md").write_text(
            "- Selma is a Python framework\n", encoding="utf-8"
        )
...
def test_search_tool_lazy_sync_on_first_call():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert "MEMORY.md" in result, (
            "Lazy sync did not index the file written after tool creation"
        )
...
async def _integration_agent_knows_memory_md():
    ...
    """
    from runtime import RuntimeEnv, agent_command
    ...
    try:
        ...
        assert "blue" in reply or "blau" in reply, (
            f"'Blue' not in reply: {reply[:200]}"
        )
    finally:
...
async def _integration_memory_flush_creates_daily_file():
    ...
    """
    from runtime import memory_flush
    ...
    try:
        ...
        assert daily_file.exists(), (
            f"Daily memory file not created by memory_flush(): {daily_file}"
        )
