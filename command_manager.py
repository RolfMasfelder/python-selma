...
from config import SelmaConfig, resolve_tools_allow, get_default_model, resolve_thinking_default, THINKING_LEVELS
from session_store import load_session_store, save_session_store, reset_session, resolve_session_file
from compaction import compact_session
from tools import ALL_TOOL_NAMES, get_tool_descriptions
from runtime import RuntimeEnv, agent_command as run_agent, memory_flush as _memory_flush_fn, DeliveryContext
from helper import get_workspace
from skills import _find_skill_files, _parse_frontmatter
...
class CommandManager:
    ...
    async def _cmd_models(self) -> str:
        from my_mono.agent_session import list_ollama_models
        try:
    ...
    def _cmd_think(self, ctx: NormalizedTurnInput, args: list[str]) -> str:
        ...
        if record:
            ...
            save_session_store(store)
            logger.info("Thinking level changed via command | session=%s level=%s",
                        session_key, level_str)
    ...
    def _cmd_status(self, ctx: NormalizedTurnInput) -> str:
        ...
        thinking = (record.thinking_level if record else None) \
