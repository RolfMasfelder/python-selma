...
### 9. ~~`runtime.py:330 agent_command(...)` — **137 Zeilen, 6 Parameter**~~ ✅ **erledigt 2026-09-19** (Commit ausstehend, vgl. `memory/2026-09-19.md`)
...
- **Verifikation:** A/B-Differential **6 Cases** (Erfolg / leeres message / Key + ID missing / Layer2-Error / `sid`-only / thinking-Override) gegen `git show HEAD:`-Backup `/tmp/old_rt.py` per **in-process Dual-Load** (Stolperstein 25 — **niemals** `git stash`) → **6/6 OK** (Exception-Typ/-Message, Lifecycle-Events, update-Store-, Deliver-Call, Result). **Suite 730 passed / 0 failed** (62 s), `runtime.py` **99 %** (413 stmts / 3 Miss L955-957), Gesamt **99 %**. `ruff check` + `ruff format --check` grün (69 Dateien), `mypy src/selma` **0 errors / 30 files** (die manuell auf 0 gebracht — keine neuen `arg-type`/`no-redef`), `find ~/.selma -type f` → **0**. `_scratch_p29/` geräumt (Stolperstein 26).
- **Stolperstein (neu, 27): ECHTE Pydantic-Modelle in Fakes** — `CommandContext` validiert `config/store/session_record/skills_snapshot` gegen die echten Typen → `MagicMock` schlägt bei der Validierung fehl. Fix im A/B-Skript: echte Instanzen (`SelmaConfig()`, `SessionStore()`, `SessionRecord()`, `SkillsSnapshot(version=…)`) importieren und beide rt-Module erhalten dieselben Klassen → gilt für beide.
- Details: `memory/2026-09-19.md`.
