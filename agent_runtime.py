...
from my_mono.agent import AgentEvent, AgentTool
from my_mono.agent_session import AgentSession, CreateSessionOptions, create_agent_session
from my_mono.system_prompt import BuildSystemPromptOptions, ContextFile, build_system_prompt
...
class RunParams(BaseModel):
    ...
    on_block_reply: Callable[[str], Awaitable[None]] | None = Field(
        default=None, exclude=True
    )
    # Called for every text chunk (message_update). Used by the SSE stream endpoint.
    on_chunk: Callable[[str], Awaitable[None]] | None = Field(
        default=None, exclude=True
    )
...
class SessionFactory:
    ...
    async def get_or_create(
        ...
        session_file = (
            Path(workspace_dir) / ".my_mono" / "sessions" / f"{session_key}.jsonl"
        )
...
class SystemPromptBuilder:
    ...
    def build(
        ...
        base = build_system_prompt(BuildSystemPromptOptions(
            cwd=workspace_dir,
                ...
                case "message_end":
                    ...
                    if msg and msg.content:
                        ...
                        if self._on_block_reply:
                            asyncio.create_task(
                                self._on_block_reply(msg.content)
                            )
...
class RunOrchestrator:
    ...
    async def run(self, params: RunParams) -> RunResult:
        ...
        logger.info("Run start | run_id=%s session=%s model=%s",
                    run_id, session_key, params.model)
    ...
    async def _execute(
        ...
        try:
            ...
            logger.info("Run ok | run_id=%s duration=%.1fs reply_len=%d",
                        run_id, duration, len(subscriber.final_reply))
            ...
            ended_at = datetime.now(timezone.utc)
            logger.warning("Run timeout | run_id=%s timeout_ms=%d",
                           run_id, params.timeout_ms)
            return RunResult(
                run_id=run_id,
                session_key=session_key,
                status="timeout",
