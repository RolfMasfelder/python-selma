...
async def compact_session(
    ...
    tokens_before = sum(len(str(getattr(m, "content", m))) for m in session.state.messages) // 4
    ...
    tokens_after = sum(len(str(getattr(m, "content", m))) for m in session.state.messages) // 4

    trace_and_log(
        logger,
        f"compact_session: compaction successful | messages {messages_before}→{messages_after} tokens_est {tokens_before}→{tokens_after}",
    )
