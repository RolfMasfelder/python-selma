...
def load_session_store(agent_id: str = DEFAULT_AGENT_ID, cwd: str = ".") -> SessionStore:
    ...
    try:
        ...
        if not isinstance(data, dict):
            logger.warning("sessions.json has invalid format (not an object), ignoring | path=%s", path)
            return SessionStore(store_path=str(path))
...
def is_session_fresh(record: SessionRecord, at_hour: int = 4, idle_minutes: int | None = None) -> bool:
    ...
    if last < boundary_utc:
        trace_and_log(
            logger, f"Session stale (daily reset) | last={last.isoformat()} boundary={boundary_utc.isoformat()}"
        )
        return False
...
def reset_session(
    ...
    if record.transcript_file:
        ...
        if old_path.exists():
            archive_dir = _sessions_dir(agent_id, cwd) / "archive" / "reset" / record.session_id
