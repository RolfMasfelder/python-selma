...
from selma.tools import get_tool_descriptions
...
def _build_assistant_output_directives_section(is_minimal: bool) -> list[str]:
    ...
    return [
        ...
        "Supported tags are stripped before user-visible rendering; support depends on the current channel config.",
        "",
    ]
...
def _build_owner_identity_line(
    ...
    if owner_display == "hash":
        display_ids = [_format_owner_display_id(oid, owner_display_secret) for oid in normalized]
    else:
...
def build_agent_system_prompt(params: BuildAgentSystemPromptParams) -> str:
    ...
    read_tool_name = next(
        (params.tool_names[i] for i, t in enumerate(params.tool_names) if t.strip().lower() == "read"), "read"
    )
    exec_tool_name = next(
        (params.tool_names[i] for i, t in enumerate(params.tool_names) if t.strip().lower() in ("exec", "bash")), "exec"
    )
    ...
    # ── Split context files ──────────────────────────────────
    valid_files = [f for f in params.context_files if f.path and f.path.strip()]
    ...
    lines += _build_project_context_section(stable_files, "# Project Context", dynamic=False)
    ...
    dynamic_heading = "# Dynamic Project Context" if stable_files else "# Project Context"
    lines += _build_project_context_section(dynamic_files, dynamic_heading, dynamic=True)
