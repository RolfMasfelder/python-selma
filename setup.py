...
def setup(agent_base_dir='.'):
    ...
    try:
        ...
        if not memory_index.exists():
            ...
            print(f"[green]✔[/green] Created memory index: [cyan]memory/MEMORY.md[/cyan]")
        else:
            print(f"[yellow]![/yellow] Memory index already exists: [cyan]memory/MEMORY.md[/cyan]")
...
def handle_templates(template_dir: Path, target_dir: Path):
    ...
    if not files_already_present:
        ...
        print("[yellow]![/yellow] Workspace already contains template files. [dim]Skipping copy to prevent overwriting.[/dim]")
...
def handle_skills(skills_src: Path, skills_dst: Path):
    ...
    if copied_skills:
        ...
        print(f"[yellow]![/yellow] All skills up to date in workspace. [dim]Skipping.[/dim]")
