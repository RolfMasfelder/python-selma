...
OpenClaw organisiert die Konversationen mit Sesseion.

Jede Nachricht wird zu einer Session geroutet, je nach dem, woher sie stammt - DMs, group chats, cron jobs, etc.
​
...
Alle DMs teilen sich einen Session wegen continuity.
Passt für Singel-User. Bei Multi-User, dann eine Session je DM

​
## Session lifecycle

**Ereignis Session-Ende**

* Daily reset (default) — Neue Session um  4:00 AM lokaler Zeit.

* Idle reset (optional) — Nache einen inaktiven Zeitraum. session.reset.idleMinutes.

* Manueller Reset — durch /new oder /reset im Chat. /new <model> ebenfalls.

​
## Session state

Der State aller Session ist beim Gateway. UI clients fragen das Gateway nach session data.
...
**session.maintenance.mode** mit *enforce* führt automatische Bereinigung durch:
```
{
  session: {
    maintenance: {
...
* Session routing (Zuordnung des sessionKey an neue Nachrichten)

* Session store (sessions.json) und dessen Inhalte
...
In OpenClaw gibt es einen Gateway Prozess, der den Session state besitzt.
UIs fragen das Gateway nach Session-Listen und Anzahl der Token
​
## Zwei persistente Layer
...
* Main/direct chat (per agent): **agent:<agentId>:<mainKey>**
...
Das Gateway verwendet **SessionManager** um es zu lesen/schreiben.
​
...
Overflow recovery: the model returns a context overflow error (request_too_large, context length exceeded, input exceeds the maximum number of tokens, input token count exceeds the maximum number of input tokens, input is too long for the model, ollama error: context length exceeded, and similar provider-shaped variants) → compact → retry.
...
Use the exact silent token NO_REPLY / no_reply so the user sees nothing.
