...
class FakeSession:
    def __init__(self, prompt_delay: float = 0.0, raise_exc: Exception | None = None, final_reply: str | None = None):
    ...
    async def prompt(self, text: str):
        ...
        return {"reply": self._final_reply}
...
class CacheFactory:
    ...
    async def get_or_create(self, session_key, workspace_dir, system_prompt, tools, model):
    ...
    def invalidate(self, session_key):
...
class TestRunResultModel:
    ...
    def test_invalid_status_rejected(self):
        ...
        with pytest.raises(ValueError):
            ar.RunResult(run_id="x", session_key="s", status="bogus", reply=None, started_at=now, ended_at=now)
...
class TestRunParams:
    ...
    def test_callbacks_excluded_from_serialization(self):
        async def cb(text):
            pass
...
class TestRunLaneManager:
    def test_lock_cached_per_session_key(self):
    ...
    def test_mark_active_done_lookup(self):
        lm = ar.RunLaneManager()
...
class TestSessionFactory:
    def test_cache_hit_updates_system_prompt_and_reuses(self, tmp_path):
        ...
        assert s1 is created and s2 is created
        assert created.agent._options.system_prompt == "NEW-PROMPT"

    def test_explicit_model_passthrough(self, tmp_path):
        factory = ar.SessionFactory()
    ...
    def test_model_none_becomes_empty_string(self, tmp_path):
        factory = ar.SessionFactory()
    ...
    def test_invalidate_removes_and_is_idempotent(self, tmp_path):
        factory = ar.SessionFactory()
...
class TestSystemPromptBuilder:
    ...
    def test_light_context_includes_heartbeat(self, tmp_path):
        ws = self._workspace(tmp_path)
        b = ar.SystemPromptBuilder()
        with mock.patch.object(ar, "build_system_prompt", side_effect=lambda o: "BASE") as m:
            prompt = b.build(
    ...
    def test_light_context_missing_heartbeat(self, tmp_path):
        ws = self._workspace(tmp_path)
        (tmp_path / "proj" / ".selma" / "workspace" / "HEARTBEAT.md").unlink()
        b = ar.SystemPromptBuilder()
        with mock.patch.object(ar, "build_system_prompt", side_effect=lambda o: "BASE"):
    ...
    def test_light_context_empty_files_no_crash(self, tmp_path):
        ws = self._workspace(tmp_path)
    ...
    def test_render_context_files_empty_list(self):
        assert ar.SystemPromptBuilder()._render_context_files([]) == ""

    def test_render_context_files_truncates(self, caplog):
        b = ar.SystemPromptBuilder()
        f = types.SimpleNamespace(path="/somewhere/FUZZ.md", content="Y" * 25_000)
    ...
    def test_full_context_uses_resource_loader(self, tmp_path, monkeypatch, caplog):
        ws = self._workspace(tmp_path)
        b = ar.SystemPromptBuilder()

        class FakeFiles(list):
            def __init__(self, items):
                super().__init__(items)

        fake = [
            types.SimpleNamespace(path="/proj/AGENTS.md", content="AGENTS-CONTENT"),
            types.SimpleNamespace(path="/proj/SOUL.md", content="SOUL-CONTENT"),
        ]
        ...
        with (
            mock.patch.object(ar, "build_system_prompt", side_effect=lambda o: "BASE") as m,
            caplog.at_level(logging.DEBUG, logger="selma.agent_runtime"),
        ):
            prompt = b.build(ws, simple_ctx(), [make_tool()], light_context=False)
        assert m.call_count == 1
        assert "### AGENTS.md" in prompt and "AGENTS-CONTENT" in prompt
        assert "### SOUL.md" in prompt and "SOUL-CONTENT" in prompt
        ...
        assert "## Safety" in prompt and "## Runtime" in prompt
...
class TestEventSubscriber:
    def test_message_update_spawns_chunk(self):
    ...
    def test_message_update_noop_without_payload_or_cb(self):
        coros, fake_spawn = _captured_spawn()
    ...
    def test_message_end_sets_final_reply(self):
        coros, fake_spawn = _captured_spawn()
        sent = []
    ...
    def test_message_end_empty_or_none_noop(self):
        coros, fake_spawn = _captured_spawn()
    ...
    def test_message_end_no_block_reply_cb(self):
        coros, fake_spawn = _captured_spawn()
        sub = ar.EventSubscriber()
        with mock.patch.object(ar, "spawn_background_task", fake_spawn):
    ...
    def test_tool_events_log_only(self, caplog):
        coros, fake_spawn = _captured_spawn()
        sub = ar.EventSubscriber()
        with (
    ...
    def test_agent_end_logs(self, caplog):
        sub = ar.EventSubscriber()
...
class TestRunOrchestrator:
    @staticmethod
    def make_orch(session: FakeSession | None = None):
        sess = session or FakeSession()
        orch = ar.RunOrchestrator()
        orch._sessions = CacheFactory(sess)
        orch._prompt_builder = types.SimpleNamespace(build=lambda **kw: "SYSPROMPT")
        return orch, sess

    @staticmethod
    def params(session_key="sk-1", **kw):
        return ar.RunParams(
    ...
    def test_empty_final_reply_becomes_none(self):
        orch, sess = self.make_orch()
        with mock.patch.object(ar, "spawn_background_task", _captured_spawn()[1]):
            result = run(orch.run(self.params()))
        assert result.status == "ok"
    ...
    def test_cache_hit_updates_system_prompt(self):
        orch, sess = self.make_orch()
        with mock.patch.object(ar, "spawn_background_task", _captured_spawn()[1]):
        ...
        assert len(orch._sessions.calls) == 2
    ...
    async def _traced_session(self):
        """Session, die prompt start/ende in timeline einträgt."""
        sess = FakeSession(prompt_delay=0.1)
        orch, _ = self.make_orch(sess)
        return orch, sess

    def test_concurrent_runs_same_session_serialize(self):
        orch, sess = self.make_orch()
        calls = []

        async def traced_prompt(text: str):
            t = asyncio.get_running_loop().time()
    ...
    def test_different_sessions_run_concurrently(self):
        ...
        async def fake_goc(session_key, **kw):
            return a if session_key == "A" else b

        orch._sessions = types.SimpleNamespace(get_or_create=fake_goc)
        orch._prompt_builder = types.SimpleNamespace(build=lambda **kw: "P")
        ...
        assert a.prompt_calls and b.prompt_calls
