...
def print_results(results: list[dict]) -> None:
    ...
    header = _pad("Model", col_model) + _pad("Tools", 8) + _pad("finish_reason", 16) + _pad("Called tool", 24) + "Error"
    sep = "─" * len(header)
    ...
    for r in results:
        ...
        error = r["error"] or ""
        print(_pad(r["model"], col_model) + _pad(status, 8) + _pad(finish, 16) + _pad(called, 24) + error)
