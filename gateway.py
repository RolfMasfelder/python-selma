...
from data import NormalizedTurnInput, WebChatIn
...
from adapter_telegram import TelegramChannel
from adapter_webchat import WebChatChannel

# load_config() reads from .selma/selma.json — single source of truth,
from config import load_config, resolve_timeout
from runtime import RuntimeEnv, agent_command as run_agent, DeliveryContext
from command_manager import CommandManager
from my_mono.tracing import tracer, setup as tracing_setup
...
async def lifespan(app: FastAPI):
    ...
    hb_task = asyncio.create_task(
        heartbeat_loop(config, ".", _pending_alerts)
    )
    try:
        yield
    finally:
